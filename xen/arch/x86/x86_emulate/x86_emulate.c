...
#undef GRP2
    ...
    case X86EMUL_OPC(0x0f, 0x07): /* sysret */
         ...
         * XSA-7 (CVE-2012-0217).
         */
        generate_exception_if((x86emul_cpu_vendor(cp) & X86_VENDOR_INTEL) &&
...
# define CASE_SIMD_SINGLE_FP_VEX(pfx, opc) CASE_SIMD_SINGLE_FP(, pfx, opc)
    ...
    case X86EMUL_OPC(0x0f, 0xbc): /* bsf or tzcnt */
        if ( vex.pfx == vex_f3 )
            emulate_2op_SrcV_srcmem("tzcnt", src, dst, _regs.eflags);
        else
        {
    ...
    case X86EMUL_OPC(0x0f, 0xbd): /* bsr or lzcnt */
        if ( vex.pfx == vex_f3 )
            emulate_2op_SrcV_srcmem("lzcnt", src, dst, _regs.eflags);
        else
        {
