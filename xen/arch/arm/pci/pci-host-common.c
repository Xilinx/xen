                                            ...
                                            uint64_t len, void *data)
{
    ...
    if ( dt_range_is_prefetchable(flags) )
    {
        ...
        base |= pci_conf_read16(sbdf, PCI_PREF_MEMORY_BASE) &
                PCI_PREF_RANGE_TYPE_MASK;
        limit |= pci_conf_read16(sbdf, PCI_PREF_MEMORY_LIMIT) &
                 PCI_PREF_RANGE_TYPE_MASK;
...
int __init pci_init_bridge(pci_sbdf_t sbdf)
{
    ...
    ret = dt_for_each_range(bridge->dt_node, set_bridge_mem_base_limit, &sbdf);
    if ( ret )
        return ret;

    /* Enable SERR# */
    val = pci_conf_read16(sbdf, PCI_BRIDGE_CONTROL);
    val |= PCI_BRIDGE_CTL_SERR;
    pci_conf_write16(sbdf, PCI_BRIDGE_CONTROL, val);

    /* Enable memory decoding and bus mastering */
    val = pci_conf_read16(sbdf, PCI_COMMAND);
    val |= PCI_COMMAND_MASTER | PCI_COMMAND_MEMORY;
    pci_conf_write16(sbdf, PCI_COMMAND, val);
...
int __init pci_get_new_bar_addr(const struct pci_dev *pdev, uint64_t size,
                                bool is_64bit, bool prefetch, uint64_t *addr)
{
    ...
    {
        ret = rangeset_remove_range(range, *addr, *addr + size - 1);
        if ( ret )
            printk(XENLOG_ERR "%s: failed to remove BAR range %d\n",
                   __func__, ret);
