...
/*
 * Mali G78AE GPU Arbiter - VM protocol header
 *
...
static inline int arb_vm_init_build_msg(uint8_t max_l2_slices,
                    uint32_t max_core_mask, uint8_t ack,
                    uint8_t version, uint64_t *payload)
{
    ...
    enc_msg |=
        ((uint64_t)max_l2_slices & MAX_L2_SLICES_MASK) << MAX_L2_SLICES_OFFSET;
    enc_msg |=
        ((uint64_t)max_core_mask & MAX_CORE_MASK_MASK) << MAX_CORE_MASK_OFFSET;
                     ...
                     uint32_t *max_core_mask)
{
    ...
    *max_l2_slices =
        (uint8_t)((*payload >> MAX_L2_SLICES_OFFSET) & MAX_L2_SLICES_MASK);
    *max_core_mask =
        (uint32_t)((*payload >> MAX_CORE_MASK_OFFSET) & MAX_CORE_MASK_MASK);
...
static inline const char* arb_vm_get_str_from_msg_id(uint8_t message_id)
{
    switch( message_id )
    {
        ...
        case ARB_VM_INIT:
            return "ARB_VM_INIT";
        default:
            return "Unknown message from ARB";
    }
}

static inline const char* vm_arb_get_str_from_msg_id(uint8_t message_id)
{
    switch( message_id )
    {
        case VM_ARB_INIT:
        ...
        case VM_ARB_GPU_IDLE:
            return "VM_ARB_GPU_IDLE";
        case VM_ARB_GPU_ACTIVE:
            return "VM_ARB_GPU_ACTIVE";
        case VM_ARB_GPU_REQUEST:
            return "VM_ARB_GPU_REQUEST";
        case VM_ARB_GPU_STOPPED:
            return "VM_ARB_GPU_STOPPED";
        default:
            return "Unknown message from VM";
    }
}

#endif /* _MALI_ARB_VM_PROTOCOL_H_ */
