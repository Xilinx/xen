...
int mali_g78ae_register_domain(struct domain *d, unsigned int aw)
{
    ...
    printk(XENLOG_DEBUG "Registering d%u with AW ID %u\n", d->domain_id, aw);
    ...
    for ( i = 0; i < MALI_PTM_PARTITION_COUNT; i++ )
    {
        ...
        if ( !err )
        {
            printk(XENLOG_DEBUG "Assigned domain %u to RG %d\n",
                   d->domain_id, i);
            return 0;
        }
    }
    printk(XENLOG_ERR "Failed to assign domain %u to any RG\n", d->domain_id);
...
static int __init mali_gpu_init(void)
{
    ...
    printk(XENLOG_DEBUG "gpu: Successfully initialized GPU interfaces\n");
