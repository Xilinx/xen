                ...
                struct mali_ptm_part_cfg *partition_configs)
{
    ...
    dt_for_each_child_node(node, child)
    {
        ...
        printk(XENLOG_DEBUG "Found res group with id: %d\n", ret);
        printk(XENLOG_DEBUG "Found res group base: 0x%"PRIpaddr"\n", rg->base);
        printk(XENLOG_DEBUG "Found res group size: 0x%"PRIpaddr"\n", rg->size);
        printk(XENLOG_DEBUG "Found res group IRQ: %d\n", rg->irq.line);
